  ...
  mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
  ...
  else
  {
    ...
    if (
      sensor == System::IMU_MONOCULAR || sensor == System::IMU_STEREO ||
...
void Tracking::PrintTimeStats()
{
  ...
  {
    if (
      pBestMap->GetAllKeyFrames().size() < vpMaps[i]->GetAllKeyFrames().size())
    {
      pBestMap = vpMaps[i];
    }
  }
...
void Tracking::newParameterLoader(Settings* settings)
{
  ...
  if (
    mSensor == System::STEREO || mSensor == System::RGBD ||
...
bool Tracking::ParseORBParamFile(cv::FileStorage& fSettings)
{
  ...
  {
    std::cerr
      << "*ORBextractor.nFeatures parameter doesn't exist or is not an integer*"
      << std::endl;
    b_miss_params = true;
  }
  ...
  {
    std::cerr
      << "*ORBextractor.iniThFAST parameter doesn't exist or is not an integer*"
      << std::endl;
    b_miss_params = true;
  }
  ...
  {
    std::cerr
      << "*ORBextractor.minThFAST parameter doesn't exist or is not an integer*"
      << std::endl;
    b_miss_params = true;
  }
...
void Tracking::PreintegrateIMU()
{
  ...
  {
    Verbose::PrintMess(
      "Not IMU data in mlQueueImuData!!", Verbose::VERBOSITY_NORMAL);
    mCurrentFrame.setIntegrated();
    return;
  }
...
EASY_BLOCK("PoseEstimation", profiler::colors::DeepPurple100);
    ...
    if (!mbOnlyTracking)
    {
      ...
      if (mState == OK)
      {
        ...
        if (
          (!mbVelocity && !pCurrentMap->isImuInitialized()) ||
        ...
        {
          Verbose::PrintMess(
            "TRACK: Track with motion model", Verbose::VERBOSITY_DEBUG);
      ...
      else
      {
        ...
        {
          Verbose::PrintMess(
            "Lost for a short time", Verbose::VERBOSITY_NORMAL);
        ...
        {
          Verbose::PrintMess(
            "A new map is started...", Verbose::VERBOSITY_NORMAL);

          if (pCurrentMap->KeyFramesInMap() < 10)
          {
            mpSystem->ResetActiveMap();
            Verbose::PrintMess(
              "Reseting current map...", Verbose::VERBOSITY_NORMAL);
          }
          else
            CreateMapInAtlas();
        ...
        else
        {
          ...
          if (bOKMM && !bOKReloc)
          {
            ...
            if (mbVO)
            {
              ...
              {
                if (
                  mCurrentFrame.mvpMapPoints[i] && !mCurrentFrame.mvbOutlier[i])
                {
...
EASY_BLOCK("TrackLocalMap", profiler::colors::DeepPurple100);
    ...
    else if (mState == OK)
    {
      ...
      {
        Verbose::PrintMess(
          "Track lost for less than one second...", Verbose::VERBOSITY_NORMAL);
...
EASY_END_BLOCK
    ...
    if (bOK || mState == RECENTLY_LOST)
    {
      ...
      // if(bNeedKF && bOK)
      if (
        bNeedKF && (bOK || (mInsertKFsLost && mState == RECENTLY_LOST &&
                            (mSensor == System::IMU_MONOCULAR ||
...
void Tracking::StereoInitialization()
{
  if (mCurrentFrame.N > 500)
  {
    if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
      ...
      if (
        !mFastInit && (mCurrentFrame.mpImuPreintegratedFrame->avgA -
    ...
    Verbose::PrintMess(
      "New Map created with " + to_string(mpAtlas->MapPointsInMap()) +
        " points",
      Verbose::VERBOSITY_QUIET);
...
void Tracking::MonocularInitialization()
{
  ...
  {
    if (
      ((int) mCurrentFrame.mvKeys.size() <= 100) ||
      ((mSensor == System::IMU_MONOCULAR) &&
...
void Tracking::CreateInitialMapMonocular()
{
  ...
  if (medianDepth < 0 || pKFcur->TrackedMapPoints(1) < 50)  // TODO Check,
                                                            // originally 100
                                                            // tracks
  {
    Verbose::PrintMess(
      "Wrong initialization, reseting...", Verbose::VERBOSITY_QUIET);
    mpSystem->ResetActiveMap();
    return;
  }
...
void Tracking::CreateMapInAtlas()
{
  ...
  if (
    mSensor == System::IMU_STEREO || mSensor == System::IMU_MONOCULAR ||
    mSensor == System::IMU_RGBD)
...
void Tracking::UpdateLastFrame()
{
  ...
  if (
    mnLastKeyFrameId == mLastFrame.mnId || mSensor == System::MONOCULAR ||
...
bool Tracking::TrackWithMotionModel()
{
  ...
  {
    Verbose::PrintMess(
      "Not enough matches, wider window search!!", Verbose::VERBOSITY_NORMAL);
    fill(
      mCurrentFrame.mvpMapPoints.begin(),
      mCurrentFrame.mvpMapPoints.end(),
      static_cast<MapPoint*>(NULL));
...
bool Tracking::TrackLocalMap()
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        Verbose::PrintMess(
          "TLM: PoseInertialOptimizationLastFrame ", Verbose::VERBOSITY_DEBUG);
...
bool Tracking::NeedNewKeyFrame()
{
  ...
  bool c4 = false;
  if (
    (((mnMatchesInliers < 75) && (mnMatchesInliers > 15)) ||
     mState == RECENTLY_LOST) &&
...
void Tracking::CreateNewKeyFrame()
{
  ...
  if (mpLastKeyFrame)
  {
    ...
    Verbose::PrintMess(
      "No last KF in KF creation!!", Verbose::VERBOSITY_NORMAL);
...
void Tracking::SearchLocalPoints()
{
  ...
  if (nToMatch > 0)
  {
    ...
    if (mState == LOST || mState == RECENTLY_LOST)  // Lost for less than 1
                                                    // second
      th = 15;  // 15
    ...
    else
    {
      ...
      else
      {
        ...
        pSolver->SetRansacParameters(
          0.99, 10, 300, 6, 0.5, 5.991);  // This solver needs at least 6 points
...
void Tracking::ResetActiveMap(bool bLocMap)
{
  ...
  {
    Verbose::PrintMess(
      "Reseting Local Mapper...", Verbose::VERBOSITY_VERY_VERBOSE);
