...
template<>
string Settings::readParameter<string>(
  cv::FileStorage& fSettings,
  const std::string& name,
  bool& found,
  const bool required)
  ...
  bNeedToResize2_(false)
{
  ...
  if (
    sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO ||
...
void Settings::readCamera1(cv::FileStorage& fSettings)
{
  ...
  if (cameraModel == "PinHole")
  {
    ...
    if (
      (sensor_ == System::MONOCULAR || sensor_ == System::IMU_MONOCULAR) &&
...
void Settings::readLoadAndSave(cv::FileStorage& fSettings)
{
  ...
  sLoadFrom_ =
    readParameter<string>(fSettings, "System.LoadAtlasFromFile", found, false);
  sSaveto_ =
...
ostream& operator<<(std::ostream& output, const Settings& settings)
{
  ...
  if (
    settings.cameraType_ == Settings::PinHole ||
  ...
  if (
    settings.sensor_ == System::STEREO ||
  ...
  if (
    settings.sensor_ == System::IMU_MONOCULAR ||
    settings.sensor_ == System::IMU_STEREO ||
