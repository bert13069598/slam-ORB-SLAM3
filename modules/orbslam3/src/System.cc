  ...
  mbShutDown(false)
{
  ...
  cout
    << endl
    << "ORB-SLAM3 Copyright (C) 2017-2020 Carlos Campos, Richard Elvira, Juan "
       "J. Gómez, José M.M. Montiel and Juan D. Tardós, University of Zaragoza."
    << endl
  ...
  {
    mpViewer = new Viewer(
      this, mpFrameDrawer, mpMapDrawer, mpTracker, strSettingsFile, settings_);
...
void System::Shutdown()
{
  ...
  {
    Verbose::PrintMess(
      "Atlas saving to file " + mStrSaveAtlasToFile, Verbose::VERBOSITY_NORMAL);
...
void System::SaveDebugData(const int& initIdx)
{
  ...
  SaveTrajectoryEuRoC(
    "init_FrameTrajectoy_" + to_string(mpLocalMapper->mInitSect) + "_" +
    to_string(initIdx) + ".txt");

  // 1. Save scale
  ofstream f;
  f.open(
    "init_Scale_" + to_string(mpLocalMapper->mInitSect) + ".txt",
    ios_base::app);
  f << fixed;
  ...
  f.open(
    "init_GDir_" + to_string(mpLocalMapper->mInitSect) + ".txt", ios_base::app);
  f << fixed;
  ...
  f.open(
    "init_CompCost_" + to_string(mpLocalMapper->mInitSect) + ".txt",
    ios_base::app);
  f << fixed;
  ...
  // 4. Save biases
  f.open(
    "init_Biases_" + to_string(mpLocalMapper->mInitSect) + ".txt",
    ios_base::app);
  f << fixed;
  ...
  f.open(
    "init_CovMatrix_" + to_string(mpLocalMapper->mInitSect) + "_" +
      to_string(initIdx) + ".txt",
    ios_base::app);
  f << fixed;
  ...
  f.open(
    "init_Time_" + to_string(mpLocalMapper->mInitSect) + ".txt", ios_base::app);
  f << fixed;
