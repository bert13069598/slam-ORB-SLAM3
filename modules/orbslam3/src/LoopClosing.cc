      ...
      if (bFindedRegion)
      {
        ...
        if (mbLoopDetected)
        {
          ...
          if (mpCurrentKF->GetMap()->IsInertial())
          {
            ...
            if (
              fabs(phi(0)) < 0.008f && fabs(phi(1)) < 0.008f &&
              fabs(phi(2)) < 0.349f)
            {
              if (mpCurrentKF->GetMap()->IsInertial())
              {
          ...
          if (bGoodLoop)
          {
            ...
            double timeLoopTotal = std::chrono::duration_cast<
                                     std::chrono::duration<double, std::milli>>(
...
bool LoopClosing::NewDetectCommonRegions()
{
  ...
  if (
    mpTracker->mSensor == System::STEREO &&
  ...
  if (!bMergeDetectedInKF || !bLoopDetectedInKF)
  {
    ...
    mpKeyFrameDB->DetectNBestCandidates(
      mpCurrentKF, vpLoopBowCand, vpMergeBowCand, 3);
#ifdef REGISTER_TIMES
  ...
  for (KeyFrame* pKFi : vpBowCand)
  {
    ...
    if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      ...
      solver.SetRansacParameters(
        0.99, nBoWInliers, 300);  // at least 15 inliers
  ...
  vector<MapPoint*>& vpMatchedMapPoints)
{
  ...
  vpMatchedMapPoints.resize(
    pCurrentKF->GetMapPointMatches().size(), static_cast<MapPoint*>(NULL));
  int num_matches = matcher.SearchByProjection(
    pCurrentKF, mScw, vpMapPoints, vpMatchedMapPoints, 3, 1.5);
...
void LoopClosing::CorrectLoop()
{
  ...
  Sophus::SE3d correctedTcw(
    mg2oLoopScw.rotation(), mg2oLoopScw.translation() / mg2oLoopScw.scale());
...
void LoopClosing::MergeLocal()
{
  ...
  if (pCurrentMap->IsInertial() && pMergeMap->IsInertial())  // TODO Check the
                                                             // correct
                                                             // initialization
  {
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs)
  {
    ...
    if (pKFi->GetMap() != pCurrentMap)
      Verbose::PrintMess(
        "Other map KF, this should't happen", Verbose::VERBOSITY_DEBUG);
    ...
    pKFi->mfScale = s;
    Sophus::SE3d correctedTiw(
      g2oCorrectedSiw.rotation(), g2oCorrectedSiw.translation() / s);
    ...
    mpTracker->mSensor == System::IMU_RGBD)
  {
    Optimizer::MergeInertialBA(
      mpCurrentKF, mpMergeMatchedKF, &bStop, pCurrentMap, vCorrectedSim3);
  }
  else
  {
...
void LoopClosing::MergeLocal2()
{
...

  {
    ...
    mpTracker->UpdateFrameIMU(
      s_on, mpCurrentKF->GetImuBias(), mpTracker->GetLastKeyFrame());
  ...
  mvpMergeConnectedKFs.insert(
    mvpMergeConnectedKFs.end(), aux.begin(), aux.end());
  if (mvpMergeConnectedKFs.size() > 6)
    mvpMergeConnectedKFs.erase(
      mvpMergeConnectedKFs.begin() + 6, mvpMergeConnectedKFs.end());
  ...
  vpCurrentConnectedKFs.insert(
    vpCurrentConnectedKFs.end(), aux.begin(), aux.end());
  if (vpCurrentConnectedKFs.size() > 6)
    vpCurrentConnectedKFs.erase(
      vpCurrentConnectedKFs.begin() + 6, vpCurrentConnectedKFs.end());
  ...
  Optimizer::MergeInertialBA(
    pCurrKF, mpMergeMatchedKF, &bStopFlag, pCurrentMap, CorrectedSim3);
  ...
  vector<MapPoint*>& vpMapPoints)
{
       ...
       mit++)
  {
    ...
    vector<MapPoint*> vpReplacePoints(
      vpMapPoints.size(), static_cast<MapPoint*>(NULL));
...
void LoopClosing::SearchAndFuse(
  const vector<KeyFrame*>& vConectedKFs,
  vector<MapPoint*>& vpMapPoints)
{
  ORBmatcher matcher(0.8);
...
EASY_BLOCK("BundleAdjustment", profiler::colors::Green);
#ifdef REGISTER_TIMES
...
EASY_END_BLOCK
  ...
  // tree
  {
    ...
    {
      Verbose::PrintMess(
        "Global Bundle Adjustment finished", Verbose::VERBOSITY_NORMAL);
             ...
             sit++)
        {
          ...
          if (pChild->mnBAGlobalForKF != nLoopKF)
          {
            ...
            if (pChild->isVelocitySet())
            {
              ...
              Verbose::PrintMess(
                "Child velocity empty!! ", Verbose::VERBOSITY_NORMAL);
