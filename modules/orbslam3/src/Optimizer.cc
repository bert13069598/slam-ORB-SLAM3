  ...
  const bool bRobust)
{
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    vSE3->setEstimate(g2o::SE3Quat(
      Tcw.unit_quaternion().cast<double>(), Tcw.translation().cast<double>()));
    vSE3->setId(pKF->mnId);
  ...
  for (size_t i = 0; i < vpMP.size(); i++)
  {
         ...
         mit++)
    {
      ...
      else if (leftIndex != -1 && pKF->mvuRight[leftIndex] >= 0)  // Stereo
                                                                  // observation
      {
        const cv::KeyPoint& kpUn = pKF->mvKeysUn[leftIndex];
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    {
      pKF->SetPose(Sophus::SE3f(
        SE3quat.rotation().cast<float>(), SE3quat.translation().cast<float>()));
    }
    else
    {
      pKF->mTcwGBA =
  ...
  bool* bHess)
{
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    {
      Verbose::PrintMess(
        "NOT INERTIAL LINK TO PREVIOUS FRAME!", Verbose::VERBOSITY_NORMAL);
      continue;
    }
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    {
      Sophus::SE3f Tcw(
        VP->estimate().Rcw[0].cast<float>(),
        VP->estimate().tcw[0].cast<float>());
      pKFi->SetPose(Tcw);
    }
    else
    {
      pKFi->mTcwGBA = Sophus::SE3f(
        VP->estimate().Rcw[0].cast<float>(),
        VP->estimate().tcw[0].cast<float>());
...
EASY_BLOCK("LocalBundleAdjustment", profiler::colors::Green);
       ...
       lit++)
  {
         ...
         mit++)
    {
      ...
      if (
        pKFi->mnBALocalForKF != pKF->mnId && pKFi->mnBAFixedForKF != pKF->mnId)
      {
         ...
         mit++)
    {
      ...
      if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
      {
        ...
        if (leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)] < 0)
        {
          ...
          leftIndex != -1 &&
          pKFi->mvuRight[get<0>(mit->second)] >= 0)  // Stereo observation
        {
          const cv::KeyPoint& kpUn = pKFi->mvKeysUn[leftIndex];
          Eigen::Matrix<double, 3, 1> obs;
  ...
  vToErase.reserve(
    vpEdgesMono.size() + vpEdgesBody.size() + vpEdgesStereo.size());
  ...
  const bool& bFixScale)
{
       ...
       mit++)
  {
         ...
         sit++)
    {
      ...
      if (
        (nIDi != pCurKF->mnId || nIDj != pLoopKF->mnId) &&
      ...
      e->setVertex(
        1,
        dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(nIDj)));
      e->setVertex(
        0,
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)
  {
         ...
         vit++)
    {
        ...
        !pKF->hasChild(pKFn) /*&& !sLoopEdges.count(pKFn)*/)
      {
        ...
        {
          if (sInsertedEdges.count(make_pair(
                min(pKF->mnId, pKFn->mnId), max(pKF->mnId, pKFn->mnId))))
            continue;
...
{
  Verbose::PrintMess(
    "Opt_Essential: There are " + to_string(vpFixedKFs.size()) +
      " KFs fixed in the merged map",
    Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess(
    "Opt_Essential: There are " + to_string(vpFixedCorrectedKFs.size()) +
      " KFs fixed in the old map",
    Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess(
    "Opt_Essential: There are " + to_string(vpNonFixedKFs.size()) +
      " KFs non-fixed in the merged map",
    Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess(
    "Opt_Essential: There are " + to_string(vpNonCorrectedMPs.size()) +
  ...
  Verbose::PrintMess(
    "Opt_Essential: vpFixedKFs loaded", Verbose::VERBOSITY_DEBUG);
  ...
  vpKFs.reserve(
    vpFixedKFs.size() + vpFixedCorrectedKFs.size() + vpNonFixedKFs.size());
  vpKFs.insert(vpKFs.end(), vpFixedKFs.begin(), vpFixedKFs.end());
  vpKFs.insert(
    vpKFs.end(), vpFixedCorrectedKFs.begin(), vpFixedCorrectedKFs.end());
  ...
  for (KeyFrame* pKFi : vpKFs)
  {
         ...
         vit++)
    {
      KeyFrame* pKFn = *vit;
      if (
        pKFn && pKFn != pParentKFi && !pKFi->hasChild(pKFn) &&
  ...
  const bool bAllPoints)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    {
      Verbose::PrintMess(
        "Sim3: Z coordinate is negative", Verbose::VERBOSITY_DEBUG);
      continue;
    }
  ...
  bool bRecInit)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    VertexPose* VP = static_cast<VertexPose*>(optimizer.vertex(pKFi->mnId));
    Sophus::SE3f Tcw(
      VP->estimate().Rcw[0].cast<float>(), VP->estimate().tcw[0].cast<float>());
    pKFi->SetPose(Tcw);
...
Eigen::MatrixXd Optimizer::Marginalize(
  ...
  bool* pbStopFlag)
{
  ...
  for (KeyFrame* pKFi : vpFixedKF)
  {
    ...
    {
      Verbose::PrintMess(
        "ERROR LBA: KF is bad or is not in the current map",
        Verbose::VERBOSITY_NORMAL);
      continue;
    }
  ...
  if (bDoMore)
  {
    ...
    Verbose::PrintMess(
      "[BA]: First optimization(Huber), there are " + to_string(badMonoMP) +
        " monocular and " + to_string(badStereoMP) + " stereo bad edges",
      Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess(
    "[BA]: Second optimization, there are " + to_string(badMonoMP) +
      " monocular and " + to_string(badStereoMP) + " sterero bad edges",
    Verbose::VERBOSITY_DEBUG);
  ...
  LoopClosing::KeyFrameAndPose& corrPoses)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
    {
      ...
      Verbose::PrintMess(
        "ERROR building inertial edge", Verbose::VERBOSITY_NORMAL);
  }
  ...
  bool bRecInit)
{
  ...
  for (size_t it = 0; it < 4; it++)
  {
    ...
    for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++)
    {
      ...
      if (
        (chi2 > chi2Mono[it] && !bClose) || (bClose && chi2 > chi2close) ||
        !e->isDepthPositive())
      {
        pFrame->mvbOutlier[idx] = true;
        e->setLevel(1);
  ...
  const map<KeyFrame*, set<KeyFrame*>>& LoopConnections)
{
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)
  {
         ...
         vit++)
    {
      KeyFrame* pKFn = *vit;
      if (
        pKFn && pKFn != pParentKF && pKFn != prevKF && pKFn != pKF->mNextKF &&
        !pKF->hasChild(pKFn) && !sLoopEdges.count(pKFn))
      {
        if (!pKFn->isBad() && pKFn->mnId < pKF->mnId)
        {
