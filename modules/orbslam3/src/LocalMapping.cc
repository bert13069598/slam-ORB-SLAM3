...
void LocalMapping::MapPointCulling()
{
  ...
  while (lit != mlpRecentAddedMapPoints.end())
  {
    ...
    else if (pMP->GetFoundRatio() < 0.25f)
    {
      ...
      ((int) nCurrentKFid - (int) pMP->mnFirstKFid) >= 2 &&
...
void LocalMapping::CreateNewMapPoints()
{
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++)
  {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++)
    {
      ...
      const cv::KeyPoint& kp2 = (pKF2->NLeft == -1) ?
                                  pKF2->mvKeysUn[idx2] :
                                  (idx2 < pKF2->NLeft) ?
                                  pKF2->mvKeys[idx2] :
      ...
      if (bStereo1)
        cosParallaxStereo1 = cos(
          2 *
          atan2(mpCurrentKeyFrame->mb / 2, mpCurrentKeyFrame->mvDepth[idx1]));
      else if (bStereo2)
      ...
      else
      {
        ...
        if (
          (errX2 * errX2 + errY2 * errY2 + errX2_r * errX2_r) >
          7.8 * sigmaSquare2)
          continue;
      }
      ...
      if (
        ratioDist * ratioFactor < ratioOctave ||
      ...
      if (mbInertial)
      {
        ...
        if (pKF->mPrevKF && pKF->mNextKF)
        {
          ...
          if ((bInitImu && (pKF->mnId < last_ID) && t < 3.) || (t < 0.5))
          {
            ...
            !mpCurrentKeyFrame->GetMap()->GetIniertialBA2() &&
            ((pKF->GetImuPosition() - pKF->mPrevKF->GetImuPosition()).norm() <
             0.02) &&
            (t < 3))
          {
            pKF->mNextKF->mpImuPreintegrated->MergePrevious(
  ...
  // Before this line we are not changing the map
  {
    ...
    if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular)
    {
      ...
      mpTracker->UpdateFrameIMU(
        mScale, vpKF[0]->GetImuBias(), mpCurrentKeyFrame);
    }
...
void LocalMapping::ScaleRefinement()
{
  ...
  if ((fabs(mScale - 1.f) > 0.002) || !mbMonocular)
  {
    ...
    mpTracker->UpdateFrameIMU(
      mScale, mpCurrentKeyFrame->GetImuBias(), mpCurrentKeyFrame);
  }
