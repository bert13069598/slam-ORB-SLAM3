...
#include <orbslam3/include/CameraModels/KannalaBrandt8.h>
#include <orbslam3/include/CameraModels/Pinhole.h>
#include <thread>
...
bool Frame::ProjectPointDistort(
  MapPoint *pMP,
  cv::Point2f &kp,
  float &u,
  float &v)
...
bool Frame::PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY)
{
  ...
  // image
  if (
    posX < 0 || posX >= FRAME_GRID_COLS || posY < 0 || posY >= FRAME_GRID_ROWS)
    return false;
...
void Frame::ComputeStereoFishEyeMatches()
{
  ...
  vector<cv::KeyPoint> stereoRight(
    mvKeysRight.begin() + monoRight, mvKeysRight.end());
