...
using DescriptorVector = std::vector;
using Descriptors = std::vector;
using FileLUT = std::vector;

bool loadDBFeatures(DescriptorVector &vfeatures, FileLUT &dbTable, const cv::Ptr &orb);

bool loadVocabulary(const std::unique_ptr &voc);

void changeStructure(const cv::Mat &plain, Descriptors &out);

bool createDatabase(const std::unique_ptr &db, const DescriptorVector &vfeatures);

bool loadQueryFeatures(DescriptorVector &vfeatures, const cv::Ptr &orb);

bool queryDatabase(const std::unique_ptr &db, const DescriptorVector &vfeatures, FileLUT &queryResultTable,
                   const FileLUT &dbTable);
...
int main() {
    ...
    auto ptrVocabulary = std::make_unique();
    ...
    auto ptrDatabase = std::make_unique(vocabulary);
    EASY_END_BLOCK;
